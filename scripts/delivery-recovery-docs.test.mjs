...
test('repository secret 문서는 S1 readiness 순서와 workflow 최소 scope를 고정한다', () => {
...
test('README 자동 배포 lead-in은 S2-ready secret과 provision 증거를 모두 요구한다', () => {
  const readmeAutomatic = section(
    readme,
    '## Cloudflare 자동 배포',
    '### 수동 복구 배포',
  ...
  assertOrderedMarkers(leadIn, 'README S2-ready prerequisites', [
  ...
  assert.doesNotMatch(
    ...
    'two Cloudflare secrets alone must not imply S2-ready automatic deployment',
  );
});
...
test('post-deploy checklist와 orphan recovery는 final gate와 exact revoke/rotate 순서를 고정한다', () => {
  ...
  assertOrderedMarkers(recovery, 'orphan session recovery', [
    ...
    ['new main deployment dispatch', 'main deploy workflow를 새로 dispatch'],
  ]);
  for (const prohibition of [
    'provision을 호출하지 않',
    '실패 run을 re-dispatch하지 않',
  ...
  assert.equal(
    ...
    0,
    'orphan recovery must refer to the S1 runbook without duplicating commands',
  );
});
