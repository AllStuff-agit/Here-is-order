...
import { PASSWORD_HASH_ITERATIONS } from '@here-is-order/identity-credential';
import {
  buildAdminSeedSql,
  generateAdminSeed,
  readAdminConfig,
...
test('현재 Worker와 같은 PBKDF2-SHA256 포맷을 생성한다', async () => {
  const expected = pbkdf2Sync(TEST_PASSWORD, TEST_SALT, 100_000, 32, 'sha256').toString('hex');
  const actual = await createPasswordHash(TEST_PASSWORD, TEST_SALT);
...
test('없는 data 디렉터리를 만들고 비공개 권한으로 seed만 기록한다', async (t) => {
  ...
  const messages = [];
  const generation = generateAdminSeed({
    env: {
  ...
  assert.ok(generation instanceof Promise);
  await generation;

  const sql = fs.readFileSync(outputPath, 'utf8');
  const passwordHash = await createPasswordHash(TEST_PASSWORD, TEST_SALT);
...
test('검증 실패 시 출력 파일이나 비밀번호를 남기지 않는다', (t) => {
  ...
  assert.throws(
    () => readAdminConfig({ ADMIN_PASSWORD: 'too-short' }),
    /12자 이상/,
  );
