...
5. API Worker 배포
6. API `GET /health` 200 및 응답 계약 확인
7. Wrangler Action의 API `deployment-url`을 `API_PROXY_URL`로 주입해 웹 Worker build/deploy
8. 웹/API proxy smoke: 웹 `GET /login` 200과 세션 없는 `GET /api/users/me` 401로 same-origin proxy 확인
...
| `api_deployed` | 새 API가 traffic 처리 | health/log를 확인하고 forward repair; schema 호환이 증명될 때만 이전 API version 검토 |
| `api_health_smoked` | 새 API public health 통과 | 동일 API URL로 web build/deploy 재시도 |
| `web_deployed` | 새 web이 traffic 처리 | proxy smoke 원인을 수정하거나 이전 web version rollback 검토; API/D1은 유지 |
