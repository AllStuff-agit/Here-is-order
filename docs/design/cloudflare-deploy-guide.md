...
GitHub Actions repository secret에는 다음 세 값을 설정합니다. 운영 무결성 감사에 권장하는 별도 read token은 6절을 따릅니다.
...
| Secret | `PRODUCTION_SMOKE_PASSWORD` | base lifecycle deployment 성공 뒤 stdin-only로 설치해 lifecycle workflow의 create-only provision step에 먼저 전달하고 create-only provision exact whitelist evidence 확인으로 initial provision readiness가 성립한 뒤에만 authenticated gate activation을 허용함. subsequent deployment에서 deploy workflow의 마지막 authenticated business smoke step과 lifecycle workflow의 provision/rotate step에만 전달하며 disable에는 전달하지 않음 |

마지막 step의 성공 증거는 `authenticated-business-smoke-v1` seven-field whitelist evidence만 허용하고 secret value는 log, summary 또는 artifact에 남기지 않습니다.
...
base lifecycle deployment가 성공한 뒤 trusted local shell에서만 repository secret을 설치한다. 각 credential block은 `set +x`로 shell tracing을 먼저 끄고 secret value를 stdout, argv, file, chat, issue, PR 또는 commit에 남기지 않는다.
...
Secret 설치가 성공한 뒤 create-only provision을 dispatch하고 그 exact run만 확인한다.
...
if [[ ! "$provision_run_url" =~ ^https://github\.com/AllStuff-agit/Here-is-order/actions/runs/([1-9][0-9]*)$ ]]; then
  exit 1
fi
...
provision_report_matches="$(gh run view "$provision_run_id" --log | rg -o '\{"operationVersion":"production-smoke-identity-operation-v1","executedAt":"[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z","databaseName":"hereisorder","action":"provision","outcome":"completed"\}')"
provision_report_count="$(printf '%s\n' "$provision_report_matches" | sed '/^$/d' | wc -l | tr -d ' ')"
test "$provision_report_count" -eq 1
unset provision_report_matches provision_report_count
```

Exact URL/run ID, main `workflow_dispatch`, workflow name/URL metadata와 exactly one five-field whitelist JSON evidence가 모두 성공한 뒤에만 initial provision readiness로 기록하고 authenticated gate activation을 시작한다. Provision은 create-only이므로 실패하거나 판정할 수 없는 run을 맹목적으로 다시 dispatch하지 않는다.

#### Credential rotation과 긴급 비활성화

Rotation은 먼저 다음 `disable` 작업으로 identity를 inactive로 만들고 모든 세션을 폐기한다. Exact disable run과 evidence가 성공하기 전에는 secret을 교체하지 않는다.
...
if [[ ! "$disable_run_url" =~ ^https://github\.com/AllStuff-agit/Here-is-order/actions/runs/([1-9][0-9]*)$ ]]; then
  exit 1
fi
...
disable_report_matches="$(gh run view "$disable_run_id" --log | rg -o '\{"operationVersion":"production-smoke-identity-operation-v1","executedAt":"[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z","databaseName":"hereisorder","action":"disable","outcome":"completed"\}')"
disable_report_count="$(printf '%s\n' "$disable_report_matches" | sed '/^$/d' | wc -l | tr -d ' ')"
test "$disable_report_count" -eq 1
unset disable_report_matches disable_report_count
```

위 exact run watch, metadata와 exactly one `action: disable` whitelist JSON report가 모든 세션 폐기를 확인한 뒤에만 별도 stdin block에서 `PRODUCTION_SMOKE_PASSWORD`를 새 48-byte credential로 교체한다.
...
새 secret 설치가 성공한 뒤 다음 `rotate` 작업으로 새 hash를 저장해 identity를 재활성화하고 그 exact run만 확인한다.
...
if [[ ! "$rotate_run_url" =~ ^https://github\.com/AllStuff-agit/Here-is-order/actions/runs/([1-9][0-9]*)$ ]]; then
  exit 1
fi
...
rotate_report_matches="$(gh run view "$rotate_run_id" --log | rg -o '\{"operationVersion":"production-smoke-identity-operation-v1","executedAt":"[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z","databaseName":"hereisorder","action":"rotate","outcome":"completed"\}')"
rotate_report_count="$(printf '%s\n' "$rotate_report_matches" | sed '/^$/d' | wc -l | tr -d ' ')"
test "$rotate_report_count" -eq 1
unset rotate_report_matches rotate_report_count
```

Exactly one `action: rotate` whitelist JSON report까지 성공해야 rotation 완료로 판정한다. Password, hash, user/session row, raw D1 envelope는 evidence가 아니다. 허용 evidence는 `production-smoke-identity-operation-v1`의 `operationVersion`, `executedAt`, fixed database name, action, `outcome: completed`뿐이다. Lifecycle run이 실패하거나 whitelist evidence가 없거나 malformed이면 authenticated gate activation과 subsequent deployment를 중단하고 raw production response를 출력하지 않은 채 진단한다.
...
- 자동 authenticated business smoke에서 `login → me → purchase-order read → logout → old-cookie 401`을 통과하고 `authenticated-business-smoke-v1` seven-field whitelist evidence가 남았는지 확인
...
인증 gate의 성공 증거는 다음 exact `seven-field whitelist`뿐이며 lifecycle의 `five-field lifecycle evidence`와 별개입니다.

| 필드 | 필수 값 |
| --- | --- |
| `smokeVersion` | `authenticated-business-smoke-v1` |
| `executedAt` | RFC3339 UTC timestamp |
| `gitSha` | 40자 lowercase Git SHA |
| `runId` | GitHub numeric run ID |
| `runAttempt` | positive integer run attempt |
| `target` | `web` |
| `outcome` | `verified` |

Password, cookie/token, identity projection, query sentinel, response header/body, business row/count, raw URL/error/exception, Cloudflare envelope는 성공과 실패 모두 log, error message, summary, artifact, evidence 또는 delivery record에 남기지 않습니다.
...
| `web_proxy_smoked` | login page와 unauthenticated proxy smoke 통과 | fixed identity authenticated business smoke 실행; 실패 우회 금지 |
...
Runner termination이 logout 전에 발생하면 orphan session이 남을 수 있습니다. Credential/session invalidation이 필요한 경우 다음 순서만 사용합니다.

1. lifecycle runbook의 `disable`을 실행하고 disable exact whitelist evidence를 확인합니다.
2. 새 secret을 stdin-only로 설치합니다.
3. `rotate`를 실행하고 rotate exact whitelist evidence를 확인합니다.
4. fresh main `workflow_dispatch`로 별도의 deployment run을 생성합니다.
5. fresh run이 성공하고 exactly one `authenticated-business-smoke-v1` seven-field report를 남겼는지 확인합니다. report의 `gitSha`, `runId`, `runAttempt`가 fresh run의 exact Git SHA, run ID, run attempt와 각각 일치해야 복구 완료입니다.

이전 실패 run의 GitHub `Re-run jobs`와 `gh run rerun`을 사용하지 않습니다. fresh run이 실패하면 orphan risk를 fail-closed로 미해결 상태로 유지합니다. 이때는 disable exact whitelist evidence 확인 → 새 secret stdin-only 설치 → rotate exact whitelist evidence 확인을 처음부터 반복한 뒤 별도의 fresh main `workflow_dispatch`를 생성합니다.

이 복구에서는 provision을 호출하지 않고 sessions row를 직접 조회하거나 출력하지 않습니다.

#### General deployment recovery

Workflow는 Worker rollback이나 D1 restore를 자동 실행하지 않습니다. authenticated business smoke가 시작됐거나 orphan risk를 배제할 수 없으면 앞의 **Orphan session recovery**만 적용합니다. 이 경우 이전 run의 GitHub `Re-run jobs`와 `gh run rerun`은 계속 금지합니다.

일반 phase-specific rerun/recovery는 authenticated business smoke가 시작되기 전 실패에만 제한합니다. 실제 Cloudflare 상태와 마지막 성공 phase를 먼저 확인한 뒤에만 위 표의 phase별 조치를 선택합니다.
...
5. Exact current `main`에서 실행된 workflow가 성공하고 `identity-compatibility-v1` JSON report가 정확히 하나 있는지 확인합니다. Report는 `auditVersion`, `executedAt`, `gitSha`, `requestId`, `legacyPasswordHashCount`, `unsupportedPasswordHashCount`, `invalidIdentityProjectionCount`, `outcome`의 여덟 필드만 이 순서로 가져야 하며, `gitSha`는 배포된 exact current `main` SHA와 같아야 합니다.
6. `unsupportedPasswordHashCount = 0`과 `invalidIdentityProjectionCount = 0`을 모두 요구합니다. `legacyPasswordHashCount`는 0이 아닐 수 있습니다.
7. Run 실패, report 누락·중복·형식 오류, SHA 불일치, zero-count gate 실패 중 하나라도 있으면 Wave 2B를 시작하지 않습니다. GitHub Actions에서 row를 조회하거나 ad hoc SQL을 실행하지 않고, 더 넓은 배포 token fallback도 사용하지 않습니다. 원인 식별과 수선은 별도로 승인된 private remediation 절차로 수행합니다.
