...
| Secret | `PRODUCTION_SMOKE_PASSWORD` | S1 merge/deploy 성공 뒤 stdin-only로 설치해 lifecycle workflow의 create-only provision step에 먼저 전달하고 create-only provision exact whitelist evidence 확인 뒤에만 S2 허용. 이후 deploy workflow의 마지막 authenticated business smoke step과 lifecycle workflow의 provision/rotate step에만 전달하며 disable에는 전달하지 않음 |
...
S1 merge deployment가 성공한 뒤 trusted local shell에서만 repository secret을 설치한다. 각 credential block은 `set +x`로 shell tracing을 먼저 끄고 secret value를 stdout, argv, file, chat, issue, PR 또는 commit에 남기지 않는다.
...
Exact URL/run ID, main `workflow_dispatch`, workflow name/URL metadata와 exactly one five-field whitelist JSON evidence가 모두 성공한 뒤에만 S1 ready로 기록하고 S2를 시작한다. Provision은 create-only이므로 실패하거나 판정할 수 없는 run을 맹목적으로 다시 dispatch하지 않는다.
...
인증 gate의 성공 증거는 다음 exact `seven-field whitelist`뿐이며 S1의 `five-field lifecycle evidence`와 별개입니다.
...
1. S1 lifecycle runbook의 `disable`을 실행하고 disable exact whitelist evidence를 확인합니다.
2. 새 secret을 stdin-only로 설치합니다.
3. `rotate`를 실행하고 rotate exact whitelist evidence를 확인합니다.
4. main deploy workflow를 새로 dispatch해 authenticated business smoke를 재검증합니다.

이 복구에서는 provision을 호출하지 않고, 실패 run을 re-dispatch하지 않으며, sessions row를 직접 조회하거나 출력하지 않습니다.
