...
새 module은 intent별 method와 공통 `PurchaseOrderResult<T>` interface를 제공한다. Hono route는 인증, HTTP 파싱, envelope 직렬화만 담당한다. D1 batch, 상태 규칙, 충돌 판정, stock movement/ledger, audit log는 module implementation 안에 둔다.
...
- Hono route를 얇은 HTTP adapter로 만든다.
...
- draft-only Order Item 변경과 soft-delete
- `creation_token`, `operation_token` 생성과 정리
...
- 추가 시 memo는 현재 동작대로 마지막 truthy 값을 사용하며 빈 문자열은 이전 memo를 덮지 않는다.
...
- migration trigger는 최종 integrity guard로 유지한다.
...
모든 intent는 다음 순서를 따른다.

1. Hono adapter가 JSON과 path를 파싱한다.
2. module이 semantic input을 검증하고 Order Item을 정규화한다.
3. module이 현재 상태를 pre-read하여 기존의 구체적인 오류 메시지를 만든다.
4. module이 상태 predicate를 포함한 D1 batch를 구성한다.
5. module이 batch를 실행한다.
6. `changes()` 결과로 stale state와 concurrent mutation을 판정한다.
...
Malformed JSON, 유효하지 않은 path ID, `POST .../items`의 container 형식 오류는 Hono adapter가 현재 오류 code와 message로 처리한다. 상태, 수량, 활성 품목, 생명주기 오류는 module이 처리한다.
...
- draft-only Order Item 변경과 삭제
- 기존 한국어 오류 message와 code
- partial receipt의 preflight와 concurrent conflict message 차이

### 10.2 Module interface tests
...
### 10.3 Hono adapter contract tests
...
- Hono route는 인증, 파싱, module 호출, 직렬화만 수행한다.
